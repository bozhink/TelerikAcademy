namespace MySqlBooks
{
    ...
    public class BooksOperationsProvider : IOperationsProvider<Book>
    {
        ...
        public void Add(Book book)
        {
            ...
            using (connection)
            {
                var command = new MySqlCommand("INSERT INTO books(Title, Author, PublishDate, ISBN) VALUES (@title, @author, @publishDate, @isbn)", connection);
        ...
        public void List()
        {
            ...
            using (connection)
            {
                var command = new MySqlCommand("SELECT Title, Author, PublishDate, ISBN FROM books", connection);
        ...
        public ICollection<Book> Search(string serachString)
        {
            ...
            using (connection)
            {
                ...
                var command = new MySqlCommand("SELECT Title, Author, PublishDate, ISBN FROM books WHERE Title LIKE CONCAT('%',@pattern,'%')", connection);
        ...
        private Book DataReaderToBook(MySqlDataReader reader)
        {
